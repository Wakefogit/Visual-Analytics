...
def fetch_last_5_records():
    ...
    global graph
    graph = []
    # Append the dictionaries as lists to the nested list
    graph.append(list(weekly_counts.values()))
    graph.append(list(monthly_counts.values()))
...
def log(request,token):
    ...
    max_id4 = vehicle_speed.objects.count()
    max_id3 = billet.objects.count()
    max_id=[max_id1,max_id2,max_id3,max_id4]
    token1 = generate_token(10)
    list = {'token1':token1,"max_id":max_id ,"username":username ,token:"token","token":t ,"last_5_records":last_5_records,"active":"active",'graph':graph}
...
def helmet(request,token1):
    ...
    list = {"table":table,'token1':token1, "username":username,"title":title,"last_5_records":last_5_records,"active":"active",'graph':graph}
...
def filter_report(request,token1):
    try:
        if request.method == 'POST':
            ...
            list = {"table": filtered_records , 'token1': token1, "username":username ,"last_5_records":last_5_records,"token":t,'graph':graph}
            return render(request, "report.html",context=list)
    except:
        table = final_report.objects.all()
        messages.success(request, 'start date and end date required')
        list = {'token1': token1, "username": username, "last_5_records": last_5_records, "token": t,"table":table,'graph':graph}
        return render(request, "report.html", context=list)
...
def live(request, token1):
        list = {'token1': token1, "username": username, "last_5_records": last_5_records, "token": t,"active4":"active",'graph':graph}
